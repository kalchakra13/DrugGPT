...
class DSDGGenerator:
    ...
    def generate_dsdg_dict(self):
        ...
        for _, row in df.iterrows():
            disease = row['Disease']
            drug = row['Drug']
            dsdg_dict[disease] = {f"Disease {cat}": row[f"Disease {cat}"] for cat in ['description', 'symptoms', 'causes', 'diagnosis', 'treatment', 'complications']}
            dsdg_dict[drug] = {f"Drug {cat}": row[f"Drug {cat}"] for cat in ['description', 'dosage', 'effects', 'toxicity', 'food_interaction', 'drug_interaction', 'pharmacodynamics']}
        return dsdg_dict
